  ...
  return (
    <Paper style={{ padding: '20px', borderRadius: '15px'}} elevation={6}>
      ...
      <div className={classes.imageSection}>
          ...
          src={post.selectedFile.length > 0 ? `http://localhost:5000/imageuploads/${post.selectedFile[0].filename}` : ''} alt={post.title} /> 
        </div>
      ...
      <div className={classes.section}>
        ...
        {recommendedPosts.map(({ title, likes, selectedFile, _id}) => (
          ...
          <div style={{ margin: '20px', cursor: 'pointer'}} onClick={() => openPost(_id)} key={_id}> 
           ...
           <img src={selectedFile.length > 0 ? `https://sweden-spectra-backend.vercel.app/imageuploads/${post.selectedFile[0].filename}` : ''} width="250" height="250"/>
          </div>
          </Grid>
        ))}
